export default {
    ...
    template: `
        ...
        <div class="places" v-if="places.length">
            <div v-for="place in places" :key="place.updated_at">
                <input type="checkbox" name="visited" value="{{ place.id }}" id="place-input" v-model="place.visited" :checked="place.visited === 1" @click.prevent="updateVisited(place )"/>
    ...
    methods: {
        ...
        updateVisited(a){
            let datas = {
                "visited": a.visited,
                "lat": parseFloat(a.showlat),
                "lng": parseFloat(a.showlng),
                "name": a.showname,
                "location_id": parseInt(this.$route.params.id)
            }
