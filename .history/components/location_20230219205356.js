export default {
    ...
    template: `
        ...
        <div class="places" v-if="places.length">
            <div v-for="place in places" :key="place.updated_at">
                    <input type="checkbox" v-model="place.visited" @change.prevent="updatePlace(place)"  />
                    <label for="place-input">{{ place.name }}</label>
                
                <div>
                    <button type="button" @click="deletePlace(place.id)">Supprimer</button>
                    <button type="button" @click="showPlace(place)">Update</button>
                </div>
            </div>
        </div>
    ...
    methods: {
        ...
        updatePlace(place){
            ...
            fetch(`${this.API_URL}/places/${place.id}`, { 
                method: "PUT",
                headers: {
                'Content-Type': 'application/json'
                },
